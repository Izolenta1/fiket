...
import { ratingParams } from "../../../model/fixtures";

const RatingContent = ({ onClose, setActiveContent }: TRatingContentProps) => {
    const { rating, setRating } = useSearchContext()
    ...
    return (
        ...
        >
            ...
            >
                {ratingParams.map(ratingData =>
                <ModalToggle
                key={ratingData.id}
                name={ratingData.name}
                condition={ratingData.value === rating}
                trueCallback={() => setRating(ratingData.value)}
                falseCallback={() => setRating("")}
                />)}
            </div>
...
export { RatingContent };

type TRatingContentProps = {
    onClose: () => void,
