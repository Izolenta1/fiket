...
import { statusParams } from "../../../model/fixtures";

const StatusContent = ({ onClose, setActiveContent }: TStatusContentProps) => {
    const { status, setStatus } = useSearchContext()
    ...
    return (
        ...
        >
            ...
            >
                {statusParams.map(statusData =>
                <ModalToggle
                key={statusData.id}
                name={statusData.name}
                condition={statusData.value === status}
                trueCallback={() => setStatus(statusData.value)}
                falseCallback={() => setStatus("")}
                />)}
            </div>
